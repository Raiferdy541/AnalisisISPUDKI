...
ISPU_THRESHOLDS = [
    {"label": "BAIK", "min": 0, "max": 50, "color": "#2ECC71"},
    {"label": "SEDANG", "min": 50, "max": 100, "color": "#F1C40F"},
    {"label": "TIDAK SEHAT", "min": 100, "max": 199, "color": "#E67E22"},
    {"label": "SANGAT TIDAK SEHAT", "min": 199, "max": 299, "color": "#E74C3C"},
    {"label": "BERBAHAYA", "min": 299, "max": 500, "color": "#8E44AD"},
]
...
def policy_card(title: str, text: str):
    """Menampilkan kartu insight/rekomendasi kebijakan."""
    st.markdown(
        f"""
...
def add_ispu_threshold_bands(fig):
    """Menambahkan band threshold kategori ISPU pada chart tren."""
    for band in ISPU_THRESHOLDS:
        fig.add_hrect(
            y0=band["min"],
            y1=band["max"],
    ...
    for value, label in [
        (50, "BAIK/SEDANG"),
        (100, "SEDANG/TIDAK SEHAT"),
        (199, "TIDAK SEHAT/SANGAT TIDAK SEHAT"),
        (299, "SANGAT TIDAK SEHAT/BERBAHAYA"),
    ]:
        fig.add_hline(
            y=value,
            line_dash="dot",
...
def build_data_dictionary() -> pd.DataFrame:
    """Menyusun data dictionary agar pengguna awam memahami variabel dataset."""
    return pd.DataFrame([
        {"Kolom": "tanggal", "Definisi sederhana": "Tanggal pengukuran kualitas udara.", "Format/Satuan": "YYYY-MM-DD", "Nilai valid / rentang seharusnya": "Tanggal valid sesuai periode data.", "Kegunaan di dashboard": "Filter periode, tren, dan pola musiman."},
        {"Kolom": "periode_data", "Definisi sederhana": "Kode periode pelaporan bulanan.", "Format/Satuan": "YYYYMM", "Nilai valid / rentang seharusnya": "Harus sama dengan tahun dan bulan pada tanggal.", "Kegunaan di dashboard": "Agregasi bulanan dan validasi konsistensi periode."},
        {"Kolom": "stasiun", "Definisi sederhana": "Nama Stasiun Pemantau Kualitas Udara atau SPKU.", "Format/Satuan": "Kategori teks", "Nilai valid / rentang seharusnya": "DKI1, DKI2, DKI3, DKI4, DKI5.", "Kegunaan di dashboard": "Filter dan perbandingan kualitas udara antar lokasi."},
        {"Kolom": "pm10", "Definisi sederhana": "Indeks partikel debu berukuran ≤10 mikrometer.", "Format/Satuan": "Indeks ISPU", "Nilai valid / rentang seharusnya": "Angka tidak negatif.", "Kegunaan di dashboard": "Parameter pembentuk max dan pencemar kritis."},
        {"Kolom": "pm25", "Definisi sederhana": "Indeks partikel halus berukuran ≤2,5 mikrometer.", "Format/Satuan": "Indeks ISPU", "Nilai valid / rentang seharusnya": "Angka tidak negatif; dapat kosong pada periode tertentu.", "Kegunaan di dashboard": "Membaca risiko partikulat halus pada periode terbaru."},
        {"Kolom": "so2", "Definisi sederhana": "Indeks Sulfur Dioksida.", "Format/Satuan": "Indeks ISPU", "Nilai valid / rentang seharusnya": "Angka tidak negatif.", "Kegunaan di dashboard": "Parameter pencemar pembentuk max."},
        {"Kolom": "co", "Definisi sederhana": "Indeks Karbon Monoksida.", "Format/Satuan": "Indeks ISPU", "Nilai valid / rentang seharusnya": "Angka tidak negatif.", "Kegunaan di dashboard": "Parameter pencemar pembentuk max."},
        {"Kolom": "o3", "Definisi sederhana": "Indeks Ozon Permukaan.", "Format/Satuan": "Indeks ISPU", "Nilai valid / rentang seharusnya": "Angka tidak negatif.", "Kegunaan di dashboard": "Membaca pencemar sekunder yang terbentuk dari reaksi NOx dan VOC dengan sinar matahari."},
        {"Kolom": "no2", "Definisi sederhana": "Indeks Nitrogen Dioksida.", "Format/Satuan": "Indeks ISPU", "Nilai valid / rentang seharusnya": "Angka tidak negatif.", "Kegunaan di dashboard": "Parameter pencemar pembentuk max dan indikasi aktivitas pembakaran."},
        {"Kolom": "max", "Definisi sederhana": "Nilai ISPU tertinggi dari seluruh parameter pencemar pada satu hari dan satu stasiun.", "Format/Satuan": "Indeks ISPU", "Nilai valid / rentang seharusnya": "Harus sama dengan nilai tertinggi dari parameter pencemar.", "Kegunaan di dashboard": "Indikator utama tingkat kualitas udara."},
        {"Kolom": "critical", "Definisi sederhana": "Parameter pencemar yang menyebabkan nilai max.", "Format/Satuan": "Kategori", "Nilai valid / rentang seharusnya": "PM10, PM2.5, SO2, CO, O3, NO2.", "Kegunaan di dashboard": "Menentukan pencemar dominan untuk fokus pengendalian."},
        {"Kolom": "categori", "Definisi sederhana": "Kategori kualitas udara berdasarkan nilai ISPU.", "Format/Satuan": "Kategori ordinal", "Nilai valid / rentang seharusnya": "BAIK 0-50, SEDANG 51-100, TIDAK SEHAT 101-199, SANGAT TIDAK SEHAT 200-299, BERBAHAYA ≥300.", "Kegunaan di dashboard": "Mengubah angka ISPU menjadi bahasa risiko yang mudah dipahami."},
    ])


def build_category_dictionary() -> pd.DataFrame:
    """Menyusun kamus kategori ISPU."""
    return pd.DataFrame([
        {"Kategori": "BAIK", "Rentang ISPU": "0–50", "Makna sederhana": "Kualitas udara relatif baik."},
        {"Kategori": "SEDANG", "Rentang ISPU": "51–100", "Makna sederhana": "Kualitas udara masih dapat diterima, tetapi tetap perlu dipantau."},
        {"Kategori": "TIDAK SEHAT", "Rentang ISPU": "101–199", "Makna sederhana": "Kualitas udara mulai berisiko bagi kesehatan."},
        {"Kategori": "SANGAT TIDAK SEHAT", "Rentang ISPU": "200–299", "Makna sederhana": "Kualitas udara berisiko tinggi dan perlu perhatian serius."},
        {"Kategori": "BERBAHAYA", "Rentang ISPU": "≥300", "Makna sederhana": "Kualitas udara sangat berbahaya dan perlu respons cepat."},
    ])


def build_pollutant_dictionary() -> pd.DataFrame:
    """Menyusun penjelasan parameter pencemar dalam bahasa awam."""
    return pd.DataFrame([
        {"Parameter": "PM10", "Penjelasan awam": "Partikel debu berukuran lebih kasar.", "Catatan interpretasi": "Dapat berkaitan dengan debu jalan, konstruksi, aktivitas tanah terbuka, pembakaran, dan sumber industri."},
        {"Parameter": "PM2.5", "Penjelasan awam": "Partikel sangat halus yang dapat masuk lebih dalam ke sistem pernapasan.", "Catatan interpretasi": "Umumnya berkaitan dengan proses pembakaran seperti kendaraan, industri, pembangkit/boiler, dan pembakaran terbuka. Perlu divalidasi dengan data sumber emisi lokal."},
        {"Parameter": "SO2", "Penjelasan awam": "Sulfur dioksida.", "Catatan interpretasi": "Sering dikaitkan dengan pembakaran bahan bakar bersulfur dan aktivitas industri tertentu."},
        {"Parameter": "CO", "Penjelasan awam": "Karbon monoksida.", "Catatan interpretasi": "Terbentuk dari pembakaran tidak sempurna, misalnya kendaraan dan aktivitas pembakaran bahan bakar."},
        {"Parameter": "O3", "Penjelasan awam": "Ozon permukaan bukan biasanya keluar langsung dari knalpot atau cerobong, tetapi terbentuk di udara.", "Catatan interpretasi": "O3 terbentuk dari reaksi NOx dan VOC dengan bantuan sinar matahari. Prekursor dapat berasal dari kendaraan, industri, pembangkit/boiler, pompa bensin, bengkel/cat, penggunaan pelarut, dan aktivitas pembakaran."},
        {"Parameter": "NO2", "Penjelasan awam": "Nitrogen dioksida.", "Catatan interpretasi": "Sering berkaitan dengan kendaraan bermotor, pembangkit, boiler, dan proses pembakaran suhu tinggi."},
    ])
...
with tabs[1]:
    ...
    fig_trend = add_ispu_threshold_bands(fig_trend)
    fig_trend.update_layout(height=540, hovermode="x unified", margin=dict(l=10, r=10, t=60, b=10))
    st.plotly_chart(fig_trend, use_container_width=True)
    peak_trend = trend.sort_values("rata_rata_ispu", ascending=False).iloc[0]
    chart_insight(
        "Insight tren temporal",
        f"Grafik tren memperlihatkan perubahan rata-rata ISPU berdasarkan granularitas {granularity.lower()}. Area warna di belakang grafik adalah threshold kategori ISPU: BAIK, SEDANG, TIDAK SEHAT, SANGAT TIDAK SEHAT, dan BERBAHAYA. Titik tertinggi pada filter aktif terjadi pada {pd.to_datetime(peak_trend[group_col]).strftime('%Y-%m-%d')} di {peak_trend['stasiun']} dengan rata-rata ISPU {format_number(peak_trend['rata_rata_ispu'])}. Jika garis tren sering masuk area TIDAK SEHAT ke atas, maka periode dan stasiun tersebut perlu menjadi prioritas pengawasan.",
    )
...
# ============================================================
# DASHBOARD 9 — DATA DICTIONARY
# ============================================================
with tabs[8]:
    st.header("Dashboard 9 — Data Dictionary")
    st.write("Menu ini menjelaskan arti setiap variabel agar dashboard mudah dipahami oleh pengguna non-teknis.")

    insight_box(
        "Tujuan menu data dictionary",
        "Data dictionary membantu pengguna memahami arti kolom, format data, rentang nilai yang seharusnya berlaku, dan kegunaannya dalam dashboard. Menu ini penting agar interpretasi tidak hanya bergantung pada istilah teknis seperti max, critical, atau categori."
    )

    st.subheader("Kamus Variabel Dataset ISPU")
    dictionary_df = build_data_dictionary()
    st.dataframe(dictionary_df, use_container_width=True, hide_index=True)

    st.download_button(
        label="⬇️ Unduh data dictionary",
        data=dictionary_df.to_csv(index=False).encode("utf-8"),
        file_name="data_dictionary_ispu_jakarta.csv",
        mime="text/csv",
    )

    st.subheader("Kamus Kategori ISPU")
    st.dataframe(build_category_dictionary(), use_container_width=True, hide_index=True)

    st.subheader("Penjelasan Parameter Pencemar")
    st.dataframe(build_pollutant_dictionary(), use_container_width=True, hide_index=True)

    chart_insight(
        "Insight penggunaan data dictionary",
        "Dengan menu ini, pengguna awam dapat memahami bahwa nilai max menunjukkan tingkat keparahan kualitas udara, critical menunjukkan pencemar dominan, dan categori menerjemahkan angka ISPU menjadi tingkat risiko yang lebih mudah dibaca."
    )


# ============================================================
# DASHBOARD 10 — INSIGHT DAN REKOMENDASI KEBIJAKAN
# ============================================================
with tabs[9]:
    st.header("Dashboard 10 — Insight dan Rekomendasi untuk Kepala DLH DKI Jakarta")
    st.write("Menu ini merangkum insight utama dan rekomendasi kebijakan yang dapat ditindaklanjuti berdasarkan data hasil filter aktif.")

    summary_avg_ispu = FILTERED["max"].mean()
    summary_unhealthy = unhealthy_pct(FILTERED)
    summary_dominant_pollutant = get_dominant(FILTERED["critical"])
    ...
    station_policy = (
        ...
        .sort_values(["pct_tidak_sehat", "rata_rata_ispu"], ascending=False)
    )
    top_policy_station = station_policy.iloc[0]
    ...
    top_policy_month = monthly_policy.iloc[0]

    pollutant_policy = (
        FILTERED.groupby("critical", as_index=False)
        .agg(jumlah=("critical", "count"), rata_rata_ispu=("max", "mean"))
        .sort_values("jumlah", ascending=False)
    )
    top_policy_pollutant = pollutant_policy.iloc[0]

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        metric_card("Rata-rata ISPU", format_number(summary_avg_ispu), "Filter aktif")
    with c2:
        metric_card("Tidak Sehat+", format_pct(summary_unhealthy), "Filter aktif")
    with c3:
        metric_card("Stasiun Prioritas", top_policy_station["stasiun"], f"Tidak Sehat+ {format_pct(top_policy_station['pct_tidak_sehat'])}")
    with c4:
        metric_card("Pencemar Dominan", summary_dominant_pollutant, "Paling sering muncul")

    st.subheader("5 Insight Utama Berbasis Data")
    policy_card("1. Risiko kualitas udara perlu dibaca dari rata-rata dan frekuensi hari Tidak Sehat+", f"Pada filter aktif, rata-rata ISPU adalah {format_number(summary_avg_ispu)} dan proporsi kategori Tidak Sehat atau lebih buruk mencapai {format_pct(summary_unhealthy)}. Pengambilan keputusan tidak cukup hanya melihat rata-rata, tetapi juga seberapa sering masyarakat terpapar hari berisiko.")
    policy_card("2. Lokasi prioritas perlu diperbarui mengikuti data", f"Stasiun dengan risiko tertinggi pada filter aktif adalah {top_policy_station['stasiun']} dengan persentase Tidak Sehat+ sebesar {format_pct(top_policy_station['pct_tidak_sehat'])}. Artinya prioritas intervensi perlu diperbarui secara berkala, bukan ditetapkan sekali untuk semua periode.")
    policy_card("3. Pencemar dominan memberi arah program pengendalian", f"Parameter {top_policy_pollutant['critical']} merupakan pencemar kritis paling sering muncul pada filter aktif. Jika suatu parameter sering menjadi critical, parameter tersebut layak menjadi fokus pengendalian emisi dan investigasi sumber pencemar.")
    policy_card("4. Pola bulanan membantu membuat kalender kewaspadaan", f"Bulan dengan rata-rata ISPU tertinggi pada filter aktif adalah {top_policy_month['nama_bulan']} dengan rata-rata ISPU {format_number(top_policy_month['rata_rata_ispu'])}. Informasi ini dapat digunakan untuk menyusun kalender risiko dan meningkatkan kesiapan operasional pada periode rawan.")
    policy_card("5. Episode ekstrem perlu diperlakukan sebagai sinyal investigasi", "Nilai ISPU yang melonjak dapat menjadi sinyal adanya kejadian khusus, perubahan meteorologi, gangguan alat, atau peningkatan sumber emisi. Episode ekstrem perlu masuk daftar prioritas investigasi harian.")

    st.subheader("Interpretasi Pencemar Dominan")
    policy_card("O3 / Ozon Permukaan", "O3 di permukaan umumnya bukan polutan yang keluar langsung dari knalpot atau cerobong. O3 terbentuk di atmosfer melalui reaksi NOx dan VOC dengan bantuan sinar matahari. Sumber prekursor NOx dan VOC dapat berasal dari kendaraan, aktivitas industri, pembangkit/boiler, pompa bensin, bengkel/cat, penggunaan pelarut, dan aktivitas pembakaran. Jadi, jika O3 dominan, interpretasinya adalah adanya kondisi yang mendukung pembentukan ozon fotokimia, bukan berarti satu sumber tunggal langsung menghasilkan O3.")
    policy_card("PM2.5 / Partikulat Halus", "PM2.5 sering berkaitan dengan proses pembakaran, seperti kendaraan bermotor, pembakaran bahan bakar industri, pembangkit/boiler, dan pembakaran terbuka. Karena ukurannya halus, PM2.5 penting diperhatikan dalam kebijakan kesehatan lingkungan. Jika PM2.5 dominan, tindak lanjut dapat diarahkan pada pengendalian emisi pembakaran dan sumber partikulat halus.")
    policy_card("PM10 / Partikulat Kasar", "PM10 dapat berkaitan dengan debu jalan, konstruksi, aktivitas tanah terbuka, pembakaran, dan sumber industri. Jika PM10 dominan, kebijakan dapat mencakup pengendalian debu konstruksi, penyapuan jalan, pengelolaan area terbuka, dan inspeksi sumber partikulat kasar.")

    st.info("Catatan interpretasi: penjelasan sumber pencemar bersifat indikatif. Untuk menyimpulkan sumber secara pasti, DLH perlu menggabungkan data ISPU dengan data meteorologi, inventarisasi emisi, lalu lintas, kegiatan industri, pengaduan masyarakat, dan hasil inspeksi lapangan.")

    st.subheader("5 Rekomendasi Kebijakan Konkret, Terukur, dan Dapat Ditindaklanjuti")
    policy_card("1. Tetapkan daftar stasiun prioritas setiap bulan", f"Gunakan indikator rata-rata ISPU, persentase Tidak Sehat+, dan tren per stasiun. Pada filter aktif, {top_policy_station['stasiun']} menjadi prioritas utama. Target operasional: daftar prioritas diperbarui minimal 1 kali setiap bulan dan menjadi dasar agenda pengawasan lapangan.")
    policy_card("2. Buat kalender kewaspadaan kualitas udara", f"Gunakan pola bulanan untuk menentukan periode rawan. Pada filter aktif, bulan {top_policy_month['nama_bulan']} memiliki rata-rata ISPU tertinggi. Target operasional: kalender risiko disusun per tahun dan digunakan untuk meningkatkan komunikasi risiko, pemantauan, dan koordinasi lintas sektor pada bulan rawan.")
    policy_card("3. Fokuskan program pengendalian pada pencemar yang paling sering menjadi critical", f"Pada filter aktif, pencemar dominan adalah {summary_dominant_pollutant}. Target operasional: setiap triwulan, DLH menyusun daftar pencemar dominan dan mengaitkannya dengan rencana pengendalian sumber emisi yang relevan.")
    policy_card("4. Bangun mekanisme peringatan dini berbasis threshold ISPU", "Gunakan ambang kategori ISPU: 100 sebagai awal Tidak Sehat, 199 sebagai awal Sangat Tidak Sehat, dan 299 sebagai awal Berbahaya. Target operasional: ketika nilai ISPU harian melewati ambang tertentu, sistem memberi notifikasi internal untuk verifikasi data, komunikasi risiko, dan investigasi awal.")
    policy_card("5. Perkuat SOP validasi data sebelum dashboard digunakan untuk keputusan", "Validasi otomatis perlu diterapkan untuk tanggal, periode_data, max, critical, dan categori. Target operasional: setiap pembaruan data memiliki cleaning log, jumlah data bermasalah, dan status validasi sebelum masuk dashboard pimpinan.")

    st.subheader("Arah Tindak Lanjut untuk Kepala DLH")
    action_box("Prioritas keputusan", "Gunakan dashboard ini untuk menjawab tiga pertanyaan pimpinan: lokasi mana yang perlu diprioritaskan, kapan periode risiko tertinggi, dan polutan apa yang perlu menjadi fokus pengendalian. Dengan begitu, rekomendasi kebijakan menjadi lebih spesifik, terukur, dan berbasis data.")
